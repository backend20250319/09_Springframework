...
public class AuthService {
    ...
    public TokenResponse login(LoginRequest request) {
        ...
        // 아이디 검증
        User user = userRepository.findByUsername(request.getUsername()).orElseThrow(() -> new BadCredentialsException("올바르지 않은 아이디 혹은 비밀번호를 입력했습니다."));

        // matches()를 이용해서 비밀번호 검증 (평문, 암호화된 값)
        if (!passwordEncoder.matches(request.getPassword(), user.getPassword())) {
            throw new BadCredentialsException("올바르지 않은 비밀번호를 입력했습니다.");
        }
        ...
        // refreshToken은 서버측에서 관리되어야 하는 데이터이고, 성능 상 추천되는 환경은 Redis 서버이다.
        ...
        RefreshToken tokenEntity = RefreshToken.builder()
                ...
                .build();
        refreshTokenRepository.save(tokenEntity); // refresh Token을 db에 저장
