...
class ParameterBindRepositoryTest {
    ...
    @DisplayName("이름을 기준으로 파라미터 바인딩 테스트")
    @Test
...
//        List<Menu> refoundList = parameterBindRepository.selectMenuByBindingName(menuName);
//        assertEquals(menuList, refoundList);
        assertEquals(menuName, menuList.get(0).getMenuName());
    }

    @DisplayName("위치를 기준으로 파라미터 바인딩 테스트")
    @Test
