...
//@TableGenerator(name = "member_seq_tbl_generator",table = "tbl_my_sequences", pkColumnValue = "my_seq_member_no")
@Access(AccessType.FIELD) // 클래스 레벨에 작성 시 모든 필드에 대한 접근 방식 결정(default:FIELD)
public class Member {
    ...
    @Transient // 잠시 제외시키고 싶을 때 사용하는 어노테이션(직렬화에서 제외되는 것임), json 필드에서도 사용
    @Column(name = "phone")
