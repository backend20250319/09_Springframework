...
@Slf4j // log 라는 변수명으로 Logger 객체 타입의 필드가 선언되고 생성된다.
public class MenuController {
    ...
    public String findMenuByCode(@PathVariable("menuCode") int menuCode, Model model) {
        // client 에서 받은 값을 service 단으로 보내기 위함
        ...
        return "menu/detail"; // view resolver 라는 객체를 통해서 /resource/menu/detail.html 로 이동
    }
...
//        log.info("log test ===> {}", test);
        ...
        //List<MenuDTO> menuLst = menuService.findMenuList();
        //log.info("log menuLst ===> {}", menuLst); // 확인용
    ...
    @GetMapping("/category")
    @ResponseBody // 응답 데이터에 body 에 반환 값을 그대로 전달하겠다는 의미(ViewResolver 사용 x)
    public List<CategoryDTO> findCategoryList(){
        return menuService.findAllCategory();
    }

    @PostMapping("/regist")
    public String registMenu(@ModelAttribute MenuDTO menuDTO) { // input의 name 값과 필드명이 동일해야한다.
        menuService.registMenu(menuDTO);
        // insert, update, delete 의 행위 후에 새로운 request, response 를 만들기 위해 redirect
