...
// Controller 어노테이션이 없으면 RequestMapping은 작동하지 않는다.
@Controller
@RequestMapping("/menu")
@RequiredArgsConstructor
@Slf4j // log라는 변수명으로 Logger객체 타입의 필드가 선언되고 생성된다.
// 요청한 Argument를 통해서 값을 가져올 수 있다.
public class MenuController {
    ...
    @GetMapping("/list")
    public String findMenuList(Model model, Pageable pageable) {
        /*
...
//        log.info("log menuList ===> {}", menuList);
    ...
    public String registMenu(@ModelAttribute MenuDTO menuDTO) { // input의 name값과 필드명이 동일해야 한다.
        ...
        // insert, update, delete의 행위후에 새로운 request, response를 만들기 위해 redirect 시킨다.
