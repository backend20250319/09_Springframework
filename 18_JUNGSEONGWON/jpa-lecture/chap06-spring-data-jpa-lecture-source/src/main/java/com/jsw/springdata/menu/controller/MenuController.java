package com.jsw.springdata.menu.controller;


import com.jsw.springdata.common.Pagenation;
import com.jsw.springdata.common.PagingButton;
import com.jsw.springdata.menu.dto.CategoryDTO;
import com.jsw.springdata.menu.dto.MenuDTO;
import com.jsw.springdata.menu.service.MenuService;
...
public class MenuController {
    ...
    public String findMenuByCode(@PathVariable("menuCode") int menuCode, Model model) {
        ...
        return "menu/detail"; // viewresolver라는 객체를 통해서 /resource/menu/detailfh dlehd
    }
    @GetMapping("/list")
    public String findMenuList(Model model,@PageableDefault Pageable pageable) {

        /* System.out.print 계열의 메소드보다 효율적으로 로그 출력을 할 수 있다.
        * 로그 레벨에 맞춘 메소드를 통해 출력처리 한다.
        * {}를 통해 값이 입력될 위치를 포메팅한다.
        * */
        log.info("log pageable ==> {}", pageable);

        /* 1. 페이징 처리안된 단순 리스트 조회 테스트 */
...
//        log.info("log menuList ==> {}", menuList);
        ...
        model.addAttribute("paging", paging); // ★ 이 한 줄이 빠져있음!
    ...
    public String registMenu(@ModelAttribute MenuDTO menuDTO){ // input의 name값과 필드명이 동일해야 한다.
        ...
        // insert, update, delete의 행위 후에 새로운 requeset, respose를 만들기 위해 redirect
