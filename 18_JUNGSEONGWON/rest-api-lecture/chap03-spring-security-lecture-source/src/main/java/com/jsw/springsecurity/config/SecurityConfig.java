package com.jsw.springsecurity.config;


import com.jsw.springsecurity.jwt.JwtAuthenticationFilter;
import com.jsw.springsecurity.jwt.JwtTokenProvider;
import com.jsw.springsecurity.jwt.RestAccessDeniedHandler;
import com.jsw.springsecurity.jwt.RestAuthenticationEntryPoint;
...
@Configuration
@EnableWebSecurity // HTTP 요청의 인증/인가 -> 처리할 수 있게
...
public class SecurityConfig {
    ...
    // 어떤 URL이 보호되고 공개되고 로그인, 로그아웃, 세션.. 기능들을 관리한다.
    @Bean
    public SecurityFilterChain filter(HttpSecurity http) throws Exception {
        ...
        http.csrf(AbstractHttpConfigurer::disable)
                ...
                .sessionManagement(session ->
                        session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) // 토큰 인증 방식 설정을 걸어주는 것이다.
                // 인증 실패, 인가 실패 핸들러
                .exceptionHandling(
                        exception ->
                ...
                .authorizeHttpRequests(auth ->
                        auth.requestMatchers(HttpMethod.POST, "/api/v1/users", "/api/v1/auth/login", "/api/v1/auth/refresh", "/api/v1/auth/logout").permitAll()
                                ...
                                .anyRequest().authenticated() // anyrequest를 하면 모든 요청을 무사 통과 시켜준다.
                )
