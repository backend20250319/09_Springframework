package com.jsw.springsecurity.auth.service;

import com.jsw.springsecurity.auth.dto.LoginRequest;
import com.jsw.springsecurity.auth.dto.TokenResponse;
import com.jsw.springsecurity.auth.entity.RefreshToken;
import com.jsw.springsecurity.auth.repository.RefreshTokenRepository;
...
import com.jsw.springsecurity.jwt.JwtTokenProvider;
...
public class AuthService {
    ...
    public TokenResponse login(LoginRequest request) {
        User user = userRepository.findByUsername(request.getUsername())
                .orElseThrow(() -> new BadCredentialsException("올바르지 않은 아이디 혹은 비밀번호를 "));
        // matches메소드를 이용해서 비밀번호 검증 ( 평문 , 암호화된 값)
        if(!passwordEncoder.matches(request.getPassword(), user.getPassword())){
            throw new BadCredentialsException("올바르지 않은 아이디 혹은 비밀번호를 ");
        }
        ...
        // refreshToken은 서버 측에서 관리되어야 하는 데이터이고 성능 상 추천되는 환경은 Redis환경이다.
        // RDBMS에 저장해서 관리하는 코드로 작성한다.
        RefreshToken tokenEntity = RefreshToken.builder()
                ...
                .build();
        refreshTokenRepository.save(tokenEntity); // refreshtoken을 데이터베이스에 저장하는
